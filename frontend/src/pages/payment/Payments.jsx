...
export default function Payments() {
  ...
  const paymentsList = Array.isArray(reduxPayments) ? reduxPayments : [];
  const loading = reduxLoading && paymentsList.length === 0;
  ...
  const filteredPayments = React.useMemo(() => {
    return paymentsList.filter((item) => {
      ...
      const boardingName = (item.boarding?.boardingName || "").toLowerCase();
      const refId = (item.transactionId || item._id || "").toLowerCase();
      const billingMonth = (item.billingMonth || "").toLowerCase();
      const type = (item.type || "").toLowerCase();
      const roomNumber = item.booking?.room?.roomNumber ? `room ${item.booking.room.roomNumber}`.toLowerCase() : "";

      const matchesSearch =
        !term ||
        boardingName.includes(term) ||
        refId.includes(term) ||
        billingMonth.includes(term) ||
        type.includes(term) ||
  ...
  }, [paymentsList, searchTerm, statusFilter]);

  const role = authUser?.role || getCurrentUser()?.role || "tenant";
  ...
  const formatDateSafely = (dateStr) => {
    ...
    try {
      ...
      return Number.isNaN(d.getTime()) ? "N/A" : format(d, "MMM dd, yyyy");
    } catch {
      return "N/A";
    }
  };
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-12 space-y-8 sm:space-y-12 overflow-y-auto min-w-0">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-slate-400 font-medium">
                        REF: {item.transactionId || String(item._id || "").substring(0, 8)}
                      </p>
                    </div>
          ...
          >
            {[
              {
                label: role === "tenant" ? "Total Paid" : "Total Collected",
                val: `Rs. ${summary.collected.toLocaleString()}`,
                growth: role === "tenant" ? "Settled" : "Completed",
                icon: CheckCircle2,
                bg: "bg-emerald-50",
                text: "text-emerald-600",
              },
              {
                label: role === "tenant" ? "Outstanding Dues" : "Pending Funds",
                val: `Rs. ${summary.pending.toLocaleString()}`,
                growth: role === "tenant" ? "Outstanding" : "Pending",
                icon: Clock3,
                bg: "bg-amber-50",
                text: "text-amber-600",
              },
              ...
              <motion.div key={i} variants={itemVariants}>
                <Card className="rounded-3xl sm:rounded-[2.5rem] border-0 shadow-lg shadow-slate-200/40 bg-white p-5 sm:p-8 group hover:shadow-2xl transition-all duration-500">
                  <div className="flex items-center justify-between mb-4 sm:mb-6">
                    <div
                      className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl ${stat.bg} ${stat.text} flex items-center justify-center group-hover:scale-110 transition-transform`}
                    >
                      <stat.icon className="w-5 h-5 sm:w-6 sm:h-6" />
                    </div>
                    <Badge className={`${stat.bg} ${stat.text} border-none font-black text-[9px] sm:text-[10px] rounded-lg px-2.5 py-1`}>
                      {stat.growth}
                    </Badge>
                  </div>
                  <div className="space-y-1">
          ...
          <Card className="rounded-3xl sm:rounded-[3rem] border-0 shadow-2xl shadow-slate-200/50 bg-white overflow-hidden">
            <CardHeader className="p-6 sm:p-10 pb-4 sm:pb-6 flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription className="text-slate-400 font-bold uppercase tracking-widest text-[9px] sm:text-[10px] mt-1">
                  Audit-ready transaction history {role === "tenant" ? "for your leases" : "across assets"}
                </CardDescription>
              </div>
            </CardHeader>
            <CardContent className="p-4 sm:p-10 pt-0 sm:pt-0">
              ...
              >
                {loading && (
                  <p className="text-center p-8 sm:p-12 text-slate-400 font-bold">Sequencing ledger data...</p>
                )}
                ...
                {filteredPayments.map((item) => {
                  ...
                  const dueDateFormatted = formatDateSafely(item.dueDate || item.createdAt);
                  const paidDateFormatted = formatDateSafely(item.paidAt);
                    ...
                    >
                      <div className="rounded-2xl sm:rounded-3xl border border-slate-100 p-4 sm:p-6 flex flex-col xl:flex-row xl:items-center justify-between gap-4 sm:gap-6 bg-slate-50/40 group hover:bg-white hover:shadow-xl transition-all duration-500 cursor-pointer">
                        <div className="flex items-center gap-4 sm:gap-6">
                          <div
                            className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl shadow-sm flex items-center justify-center shrink-0 transition-colors ${
                              item.status === "completed"
                                ? "bg-emerald-50 text-emerald-600"
                                : isOverdue
                          ...
                          <div className="min-w-0 space-y-1">
                            ...
                            <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest truncate">
                              REF: {item.transactionId || String(item._id || "").substring(0, 8)} •{" "}
                              {item.boarding?.city || "Stay"}
                              {item.booking?.room?.roomNumber ? ` • RM ${item.booking.room.roomNumber}` : ""}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center justify-between xl:justify-end gap-4 sm:gap-8 border-t xl:border-t-0 pt-4 xl:pt-0 w-full xl:w-auto">
                          ...
                          <div>
                            {isOverdue ? (
                              ...
                              <Badge
                                className={`font-black tracking-widest text-[9px] sm:text-[10px] rounded-lg px-2.5 py-1 border-none uppercase ${
                                  item.status === "completed"
                                    ? "bg-emerald-50 text-emerald-600"
                                    : item.status === "failed"
                                    ? "bg-rose-50 text-rose-600"
                          ...
                          {role !== "tenant" && item.status === "pending" ? (
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleStatusUpdate(item._id, "failed");
                                }}
                                variant="ghost"
